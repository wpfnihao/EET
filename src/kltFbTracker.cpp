...
kltFbTracker::track(void)
{
	...
	float th = 0.5;
	findStableFeatures(initFeatures, initFeatures2d, bFeatures, cFeatures, fStatus, bStatus, stableFeatures3d, stableFeatures2d, th);	
	
	// Step 5: measure the new pose
	measurePose(stableFeatures3d, stableFeatures2d);
...
void
kltFbTracker::measurePose(std::vector<vpPoint>& stableFeatures3d, std::vector<cv::Point2f>& stableFeatures2d)
{
	vpPoseFeatures pose;
